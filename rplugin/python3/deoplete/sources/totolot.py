...
if os.path.exists(d1):
    ...
    ruby = open(os.path.expanduser("~/.cache/dein/repos/github.com/takkii/ruby-dictionary3/autoload/source/ruby_method_deoplete"))
else:
...
class Source(Base):
    ...
    def gather_candidates(self, context):
        try:
            dic = data_ruby
            dic.sort(key=lambda dic:int(dic.split()[0]))
            return dic
        except Exception:
            traceback.print_exc()
